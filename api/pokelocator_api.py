...
def set_location(location_name=None, lat=None, lng=None):
    if lat and lng:
        print('[!] You specified coordinates!')
        print('[!] lat/long/alt: {} {} {}'.format(lat, lng, 0.0))
        set_location_coords(lat,lng, 0.0)
    elif location_name:
        geolocator = GoogleV3()
...
def main(location=None, lat=None, lng=None, creds={}):
    
    pokemons = json.load(open('api/pokemon.json'))
    ptc_username = creds.get('PTC_USERNAME', "Invalid")
    ptc_password = creds.get('PTC_PASSWORD', "Invalid")
            
    set_location(location, lat, lng)
    ...
    if api_endpoint == "https:///rpc":
        ...
        login_type = "ptc"
        try:
            if ptc_username == "Invalid": raise Exception
        ...
        if access_token is None:
            ...
            goog_username = creds.get('GOOG_USERNAME', "Invalid")
            goog_password = creds.get('GOOG_PASSWORD', "Invalid")
    ...
    for i in range(1):
        ...
        for poke in visible:
            ...
            nearby_pokes.append("%i,%s,%s,%s,%s,%s" % (int(time.time()), poke.pokemon.PokemonId, pokemons[poke.pokemon.PokemonId - 1]['Name'], poke.Latitude, poke.Longitude, poke.TimeTillHiddenMs / 1000))
